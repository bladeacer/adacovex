...
presentation (css/dashboard.css, the vendored charts.min.css, and the
charts.css patch css/charts-patch.css layered on top of it) and behaviour
the vendored graphre.js / nomnoml.js / flexsearch.js plus the authored
js/ modules (theme, tabs, deps, details, nomnoml diagram, search).
...
base CSS, then the vendored Charts.css, then the patch CSS on top; the
vendored libraries first, then the authored modules in dependency order
(theme, tabs, deps, details, nomnoml, search).
...
MODULES: Dict[str, str] = {
    ...
    "__STYLE_PATCH__": "css/charts-patch.css",  # author patch over Charts.css (minified)
...
def generate(out: Path, template: Path) -> None:
    ...
    header: str = (
        ...
        " charts.min.css,\n"
        "--  css/charts-patch.css, js/theme.js, js/tabs.js, js/deps.js,"
        " js/details.js,\n"
        "--  js/nomnoml.js, js/search.js, graphre.js, nomnoml.js,"
        " flexsearch.js) --\n"

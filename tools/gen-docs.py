...
2. collects the built site into an offline asset set -- every HTML page, the
   ...
   * every remaining non-ASCII glyph is escaped as an HTML character
   ...
   (`searchindex-<hash>.js`) is a multi-MB single line, far past the ~1 MB
   per-constant limit of the gnatprove frontend, so it is split into fixed-
   size chunks, each its own Ada constant, and the server streams the chunks
   back as one response when the page requests the index;
...
# fonts, the PNG screenshots, and the print/404 pages.  mdbook's search
# machinery (elasticlunr-, searcher-, mark-, searchindex-) IS bundled so the
# manual's own search works offline.
...
def generate(out: Path) -> None:
    ...
    # The multi-MB mdBook search index becomes several smaller constants; every
    # other asset is one constant.  plan = [(rel, mime, [chunk, ...]), ...].
    plan: List[Tuple[str, str, List[str]]] = []
    for rel, mime, body in assets:
        if rel.startswith("searchindex-"):
    ...
    lines.append("   --  one body except the search index, which is chunked so")
    lines.append("   --  each constant stays under the gnatprove limit.")
    if chunked_at:

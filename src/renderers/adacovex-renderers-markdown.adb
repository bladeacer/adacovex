...
package body Adacovex.Renderers.Markdown is
   ...
   begin
      ...
      Ada.Text_IO.Put_Line (F, "|--------|-------|");
      Ada.Text_IO.Put_Line (F, "| Packages Scanned | " & Natural'Image (Pkg_Count) & " |");
      Ada.Text_IO.Put_Line (F, "| Total Subprograms | " &
                            Natural'Image (Doc_Metrics.Total_Subprograms) & " |");
      Ada.Text_IO.Put_Line (F, "| Documented Subprograms | " &
                            Natural'Image (Doc_Metrics.Documented_Subprogs) & " |");
      Ada.Text_IO.Put_Line (F, "| Docstring Coverage | " &
   ...
   begin
      ...
      for P in 1 .. Pkg_Count loop
         if Packages (P).Total_HLR_Tags > 0 then
            Ada.Text_IO.Put (F, "| " & Packages (P).Name (1 .. Packages (P).Name_Len) & " | ");
            for T in 1 .. Packages (P).Total_HLR_Tags loop
               if T > 1 then
                  Ada.Text_IO.Put (F, ", ");
               end if;
                Ada.Text_IO.Put (F, Packages (P).HLR_Tags (T).Tag (1 .. Packages (P).HLR_Tags (T).Len));
            end loop;
            Ada.Text_IO.Put_Line (F, " |");
         end if;
      end loop;

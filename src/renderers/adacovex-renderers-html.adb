...
--  SPDX-License-Identifier: Apache-2.0
   ...
   is
    ...
    begin
      ...
      Put ("<style>");
      Put ("body{font-family:DejaVu Sans,sans-serif;margin:40px;background:#f5f5f5}");
      Put ("h1{color:#333}");
      Put (".card{background:#fff;border-radius:8px;padding:20px;margin:16px 0;box-shadow:0 2px 4px rgba(0,0,0,.1)}");
      Put ("table{width:100%;border-collapse:collapse}");
      Put ("th,td{padding:8px 12px;text-align:left;border-bottom:1px solid #ddd}");
      ...
      -- Badges
      Put ("<div class=""card""><h2>Status Badges</h2><div class=""badge-container"">");
      ...
      for C in 1 .. Tests.Category_Count loop
         Put ("<tr><td>");
         Put (Tests.Categories (C).Category (1 .. Tests.Categories (C).Cat_Len));
         Put ("</td><td>");
         Put (Natural'Image (Tests.Categories (C).Test_Count));
         Put ("</td><td class=""");
         Put (if Tests.Categories (C).Status = Types.Pass then "pass" else "fail");
         Put (""">");
      ...
      if DAL_Assess.Failed_Count > 0 then
         for R in 1 .. DAL_Assess.Failed_Count loop
            ...
            Put (DAL_Assess.Failed_Reasons (R) (1 .. Types.Max_Desc_Str));
            Put ("</td></tr>");
         end loop;
      end if;
         ...
         if Packages (P).Total_HLR_Tags > 0 then
            ...
            for T in 1 .. Packages (P).Total_HLR_Tags loop
               if T > 1 then
                ...
                Put (Packages (P).HLR_Tags (T).Tag (1 .. Packages (P).HLR_Tags (T).Len));
            end loop;
            Put ("</td></tr>");
         end if;
      end loop;
      ...
      Put ("<p style=""color:#888;font-size:0.8em"">Generated by adacovex</p>");
      Put ("</body></html>");

...
package Adacovex.Prove_Patch is
   ...
   --  The patched source is Original with every subprogram declaration that
   ...
   --  success OK is True and Merged (1 .. Merged_Len) is the patched
   --  source; on failure (a patched subprogram has no match in the
   --  original, or the merged source would overflow the buffer) OK is
   --  False and the result is undefined -- the caller skips the patch and
   --  reports.
   ...
   --  spec and body with its merged form, so GNATprove runs against a
   --  faithful replica whose vendored sources carry the patch contracts.
   --  The root project file of the copy is returned in Copy_GPR --
   --  Run_Prove passes it to gnatprove and copies the resulting
   --  gnatprove.out back to <target>/obj/gnatprove/gnatprove.out.  The
   --  copy lives under the target's obj/ so it is excluded from scanning,
   --  manifest graphs, and the prove input hash (patches are hashed
   --  separately).
   --  @param Target_Dir  Target project root.

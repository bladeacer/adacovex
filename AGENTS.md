...
- Platinum SPARK level (408 VCs under gnatprove 16.1.0, 0 unproved, 0
  justified; see `docs/proof/16.1.0-ledger.md`)
- 659/659 native tests passing
...
`-- tests/
    ...
    |-- adacovex_config_tests.ads/.adb        -- CLI config tests (124)
    ...
    |-- adacovex_renderer_tests.ads/.adb      -- HTML/Markdown renderer tests (34)
    ...
    `-- test_runner.adb                       -- Test suite entry point (659 tests)
```
<!-- agents-tree:end -->
...
| `test` | Build + run the 659-test native suite |
...
| SPARK proof | `make prove` | Platinum (408 VCs, 0 unproved, 0 justified under gnatprove 16.1.0) |
...
Native zero-dependency suite (`src/tests/`, 659 tests across 12 categories).

...
- the release changelog under `docs/changelogs/` when the change ships in a
  release;
- `docs/ste100-technical-names.md` when a new Technical Name enters the docs
  or docstrings.
...
`docs/cli-reference.md` (as a flag or as a subcommand), every dashboard
route the server dispatches on appears in `docs/dashboard.md`, and every
action input has a row in the `docs/ci-cd.md` `### Inputs` table.

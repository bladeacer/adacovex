...
- **User documentation** (`docs/*.md` except `api-docs/`) -- written for end
  ...
  The `docs/book/` build output is generated by `mdbook build` and committed
  with the source (the `.gitignore` entry was dropped in 1.38.0), so
  regenerate it with `make book` whenever `docs/` changes and keep the
  committed build output in sync.

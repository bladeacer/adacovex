...
- Platinum SPARK level (401 VCs under gnatprove 16.1.0, 0 unproved, 0
  justified; see `docs/proof/16.1.0-ledger.md`)
- 616/616 native tests passing
...
`-- tests/
    ...
    |-- adacovex_config_tests.ads/.adb        -- CLI config tests (97)
    ...
    |-- adacovex_renderer_tests.ads/.adb      -- HTML/Markdown renderer tests (21)
    ...
    `-- test_runner.adb                       -- Test suite entry point (616 tests)
```
<!-- agents-tree:end -->
...
| `test` | Build + run the 616-test native suite |
...
| SPARK proof | `make prove` | Platinum (401 VCs, 0 unproved, 0 justified under gnatprove 16.1.0) |
...
Native zero-dependency suite (`src/tests/`, 616 tests across 12 categories).

...
adacovex uses a native zero-dependency test framework (`Adacovex.Test_Support`) with 555 tests across 12 categories. No external test framework (AUnit, etc.) is required. Test results are written to `docs/test_result.md` in a parseable Markdown table format.

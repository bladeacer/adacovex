...
- Sphinx runs only when the docs sources changed, and a changed tree
  rebuilds incrementally. Only the changed pages are re-read (their mtimes
  are refreshed first, so a stale doctree can never be served), the outputs
  of a removed page are deleted, and a navigation change (a page added or
  removed, or a `{toctree}` edit) rewrites every page so no sidebar keeps a
  stale entry. The page set is checked against the sources afterwards, and a
  failure falls back to a clean rebuild; `python3 tools/gen-docs.py --fresh`
  forces one.
- The incremental build is a pure speed-up: a one-page prose edit costs about
  1.2 s of Sphinx time instead of about 7 s, and the emitted spec is
  byte-identical to a clean build of the same sources.

...
The tree's typography is tightened: `line-height` 1.5 -> 1.65, node margins 4px -> 6px, leaf padding 6x8 -> 8x12, gaps 8px -> 10px, and a hover `border-color: var(--accent)` transition. Every `<li class="dep-node">` now carries `data-scope` (`base` / `dev` / `transitive` / `vendored`) and `data-name`, and the toolbar gains four scope checkboxes (base, dev, transitive, vendored -- all checked by default) plus `Expand all` / `Collapse all`. A new `filterByScope()` JS function composes the name filter and the scope filters so `dev` and `vendored` deps can be hidden without losing the hierarchy (and re-renders the nomnoml diagram when visible). The `dep-filter` input is now visually consistent with the global search box (`padding:7px 12px; border-radius:8px; font-size:.88rem`).

The **Overview** tab is also more useful: after the Source Overview / Quick Stats grid it now shows a collation `chart-grid` with three mini visuals (SPARK donut, Tests pie, Docs bar) so the key pies/bars are visible without leaving the overview. The same `data-scope` attribute drives the scope pie in C3.
...
The dashboard header gains a global search box (packages, HLRs, deps) powered by vendored [FlexSearch 0.7.31](https://github.com/nextapps-de/flexsearch) (Apache-2.0, `resources/flexsearch.js`, 16 KB, inlined before the theme/tab script). At page load the index is hydrated robustly for both FlexSearch 0.7 (`FlexSearch. Index`) and 0.8 (`FlexSearch` as constructor) APIs: `FlexSearch. Index({tokenize:'forward'})` with a `try` fallback, populated from `ADACOVEX_GRAPH.dependencies` and from rendered `data-name` attributes.

Queries are served from the index (20-hit limit) with a DOM fallback, and hits appear in a `search-hits` dropdown (absolute, `max-height:240px`, `z-index:10`) that is keyboard-focusable and jumps to the Dependencies tab while seeding `dep-filter`. The header is now responsive: `flex:1 1 200px` search box with `@media(max-width:640px)` stacking, and the `dep-filter` input in the Dependencies toolbar is unified to the same `padding:7px 12px; border-radius:8px; font-size:.88rem` as the global box (previously `8px 10px / .9rem` vs `.88rem`). See `docs/dashboard.md` and `docs/THIRD_PARTY_NOTICES.md`.

### C6: Man pages are now single source of truth from source

`Adacovex. Renderers. Man` no longer hard-codes the `. SH OPTIONS` list. `Render_Page` now derives the option list at runtime from `Adacovex.

Config. Flag_List` (the same `Known_Flags` that drives `--help` and shell completion), iterating the space-separated flag table and emitting a `. TP` entry per flag via a central `Desc_For` map. Adding a flag to `Known_Flags` automatically adds a man entry (with a fallback generic line) even before a bespoke description is added, so the man page, `--help`, and completion can never drift.

A future `tools/gen-man.py` generator (parity with `gen-dashboard.py` / `gen-version.py`) is documented as the next step, but the runtime derivation already makes `Flag_List` the single source. The `man --check` / `man --dir` / `man --force` flow and the installed page header (`adacovex vX. Y. Z`) are unchanged.

...
The release workflow runs the same gate against the last release tag (a
maintainer-only step; the [developer guide](../contributing/developer-guide.md) covers it).
For example, `--coverage-delta=v1.1.0` aborts if docstring coverage regressed
between two tags you compare at the command line.

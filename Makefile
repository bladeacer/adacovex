.PHONY: help build test prove doc clean run-self run-ada-crdt \
        ...
        release publish test-publish _dev_cmd agents-tree sbom
...
prove: build
	SOURCE_DATE_EPOCH=$$(git show -s --format=%ct HEAD 2>/dev/null || echo 0) ./bin/adacovex prove --target=. --dal=C --gnatprove-version=16.1.0 --require-spark=Platinum --require-docstrings=100 --require-tests=360 --require-proof=100 --emit-svg=docs/badges/
...
run-self: build
	SOURCE_DATE_EPOCH=$$(git show -s --format=%ct HEAD 2>/dev/null || echo 0) ./bin/adacovex --dal=C --require-spark=Platinum --require-docstrings=100 --require-tests=360 --require-proof=100 --emit-svg=docs/badges/
...
release:
	...
	SOURCE_DATE_EPOCH=$$(git show -s --format=%ct HEAD 2>/dev/null || echo 0) ./bin/adacovex --target=. --dal=C --require-spark=Platinum --require-docstrings=100 --require-tests=360 --require-proof=100 --emit-svg=docs/badges/; \
